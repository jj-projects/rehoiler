...
- (IBAction)connectButton:(NSButton *)sender {
   ...
   if ([sender.title isEqualToString:@"Verbinden"]) {
      ...
      if ([_port isOpen]) {
         ...
         [alert setMessageText:@"Verbindung nicht möglich"];
         [alert setInformativeText:@"Vergewissern Sie sich, dass er Treiber installiert wurde und Sie die richtige Schnittstelle ausgewählt haben."];
         [alert setAlertStyle:NSInformationalAlertStyle];
         [alert beginSheetModalForWindow:[self window]
                       completionHandler:^(NSInteger result) {
                          NSLog(@"Driver not installed");
                       }];
      }
   } else {
      [_port close];
...
-(void)serialPort:(ORSSerialPort *)serialPort didReceiveData:(NSData *)data {
   ...
   if ([_receivedString hasSuffix:@"\r\n"]) {
      ...
      if ([self isProgramming]) {
         NSAlert *alert = [[NSAlert alloc] init];
         [alert addButtonWithTitle:@"OK"];
         [alert setMessageText:@"Programmiert"];
         [alert setInformativeText:@"Programmierung war erfolgreich"];
         [alert setAlertStyle:NSInformationalAlertStyle];
         [alert beginSheetModalForWindow:[self window]
                       completionHandler:^(NSInteger result) {
                          NSLog(@"Success");
